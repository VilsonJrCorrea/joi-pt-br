...
module.exports = {
    ...
    'alternatives.base': '"{{#label}}" não corresponde com nenhum dos tipos permitidos',
    'alternatives.types': '"{{#label}}" deve ser um dos tipos válidos {{#types}}',
    'alternatives.match': '"{{#label}}" não corresponde com nenhum dos tipos permitidos',

    'array.base': '"{{#label}}" deve ser um array',
    'array.includes': '"{{#label}}" não corresponde com nenhum dos tipos permitidos',
    'array.includesRequiredUnknowns': '"{{#label}}" não contem {{#unknownMisses}} os valores obrigatórios',
    'array.includesRequiredKnowns': '"{{#label}}" não contem {{#knownMisses}}',
    'array.includesRequiredBoth': '"{{#label}}" não contem {{#knownMisses}} e {{#unknownMisses}} os outros valores obrigatórios',
    'array.excludes': '"{{#label}}" contem um valor excluído',
    'array.hasKnown': '"{{#label}}" não contem pelo menos uma das correspodências obrigatórias para o tipo "{#patternLabel}"',
    'array.hasUnknown': '"{{#label}}" deve ter pelo menos uma das correspondências obrigatórias',
    ...
    'array.ref': '"{{#label}}" referência "{{#ref}}" não é um valor inteiro positivo',
    'array.sort': '"{{#label}}" deve ser organizado {#order} ordenado por {{#by}}',
    'array.sort.mismatching': '"{{#label}}" não pode ser ordenado devido aos tipos incompatíveis',
    'array.sort.unsupported': '"{{#label}}" não pode ser ordenado devido ao tipo incompatível {#type}',
    ...
    'binary.ref': '"{{#label}}" referência "{{#ref}}" não é um valor inteiro positivo',

    'date.base': '"{{#label}}" deve ser um número em milissegundos ou uma data valida',
    ...
    'date.timestamp.javascript': '"{{#label}}" deve ser um timestamp ou número em milissegundos',
    'date.timestamp.unix': '"{{#label}}" deve ser um timestamp ou o número de segundos',
    ...
    'object.and': '"{{#label}}" contem {{#presentWithLabels}} sem os pares obrigatórios {{#missingWithLabels}}',
    'object.assert': '"{{#label}}" não é valido pois "{{#ref}}" falha na {{#message}}',
    ...
    'object.missing': '"{{#label}}" deve ter pelo menos um dos {{#peersWithLabels}}',
    'object.nand': '"{{#mainWithLabel}}" não deve ter simultâneamente pares com {{#peersWithLabels}}',
    'object.oxor': '"{{#label}}" existem conflito entre pares opcionais e exclusivos {{#peersWithLabels}}',
    'object.pattern.match': '"{{#label}}" as chaves não correspondem aos requisitos padrões',
    'object.ref': '"{{#label}}" referências "{{#ref}}" não é um número inteiro positivo',
    'object.refType': '"{{#label}}" deve ser uma refência do Joi',
    'object.rename.multiple': '"{{#label}}" não pode ser renomeado "{{#from}}" pois multiplas renomeações estão desabilitadas e as chaves já estão renomeadas para "{{#to}}"',
    'object.rename.override': '"{{#label}}" não pode ser renomeado "{{#from}}" pois a sobreescrita está desabilitada e o objeto "{{#to}}" existe',
    'object.schema': '"{{#label}}" deve ser um esquema do Joi com o {{#type}} tipo',
    'object.with': '"{{#mainWithLabel}}" faltam pares obrigatórios "{{#peerWithLabel}}"',
    'object.without': '"{{#mainWithLabel}}" conflito com pares proíbidos "{{#peerWithLabel}}"',
    'object.xor': '"{{#label}}" existe um conflito entre pares exclusivos {{#peersWithLabels}}',
    ...
    'string.token': '"{{#label}}" deve ter somente caracteres alfanuméricos e sublinhados',
    'string.regex.base': '"{{#label}}" com valor "{[.]}" não corresponde ao padrão obrigatório: {{#pattern}}',
    'string.regex.name': '"{{#label}}" com valor "{[.]}" não corresponde o padrão {{#name}}',
    'string.regex.invert.base': '"{{#label}}" com valor "{[.]}" corresponde ao padrão invertido: {{#pattern}}',
    'string.regex.invert.name': '"{{#label}}" com valor "{[.]}" corresponde ao padrão invertido: {{#name}}',
    ...
    'string.uriCustomScheme': '"{{#label}}" deve ser um uri com esquema correspondente ao {{#scheme}} padrão',
    'string.isoDate': '"{{#label}}" deve ser uma data valida no padrão ISO 8601',
    'string.isoDuration': '"{{#label}}" deve ser uma duração valida no padrão ISO 8601',
    'string.guid': '"{{#label}}" deve ser um GUID valido',
    'string.hex': '"{{#label}}" deve ter somente caracteres hexadecimais',
    'string.hexAlign': '"{{#label}}" representação descodificada hexadecimal deve estar alinhada por byte',
    ...
    'string.normalize': '"{{#label}}" deve ser um unicode normalizado no formulário {{#form}}',
    'string.lowercase': '"{{#label}}" deve ser somente letras minúsculas',
    'string.uppercase': '"{{#label}}" deve ser somente letras maiúsculas',
    'string.trim': '"{{#label}}" não deve ter caracteres em branco à esquerda ou à direita',
    'string.creditCard': '"{{#label}}" deve ser um cartão de crédito',
    'string.ref': '"{{#label}}" referência "{{#ref}}" não é um número',
    'string.ip': '"{{#label}}" deve ser um endereço de ip valido com a {{#cidr}} CIDR',
    'string.ipVersion': '"{{#label}}" deve ser um endereço de ip valido de uma das versões {{#version}} com a {{#cidr}} CIDR',
